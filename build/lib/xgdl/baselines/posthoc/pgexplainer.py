...
class PGExplainer(BaseRandom):

    def __init__(self, clf, extractor, criterion, config):
        super().__init__()
        self.name = 'pgexplainer'
        self.clf = clf
    ...
    def __loss__(self, mask, clf_logits, clf_labels, epoch, batch):
    ...
    def forward_pass(self, data, epoch, do_sampling):
        ...
        except: # for post-hoc explaining inherent models
            emb, edge_index = self.clf.clf.get_emb(data) 
        node_mask_log_logits = self.extractor(emb, batch=data.batch, pool_out_lig=None)
        ...
        loss, loss_dict = self.__loss__(node_mask_log_logits.sigmoid(), masked_clf_logits, original_clf_logits.sigmoid(), epoch, data.batch)
        return loss, loss_dict, original_clf_logits, node_mask.reshape(-1)
