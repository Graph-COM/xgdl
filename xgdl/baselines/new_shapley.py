...
class MarginalSubgraphDataset(Dataset):
    ...
    def __init__(self, data, exclude_mask, include_mask, subgraph_build_func):
        ...
        self.label = data.y
        self.exclude_mask = torch.LongTensor(exclude_mask).to(self.device)
        self.include_mask = torch.LongTensor(include_mask).to(self.device)
    ...
    def get(self, idx):
        # exclude_graph_X, exclude_pos, exclude_graph_edge_index = self.subgraph_build_func(self.data, self.exclude_mask[idx])
        # include_graph_X, include_pos, include_graph_edge_index = self.subgraph_build_func(self.data, self.include_mask[idx])
        exclude_data, include_data = copy.copy(self.data), copy.copy(self.data)
        exclude_data.node_mask, include_data.node_mask = self.exclude_mask[idx], self.include_mask[idx]
        # exclude_data = Data(x=exclude_graph_X, edge_index=exclude_graph_edge_index, pos=exclude_pos, node_mask=self.exclude_mask[idx])
        # include_data = Data(x=include_graph_X, edge_index=include_graph_edge_index, pos=include_pos, node_mask=self.include_mask[idx])
...
def graph_build_zero_filling(data, node_mask: torch.Tensor):
    ...
    if data.pos is None:
        ...
        pos = data.pos * node_mask.unsqueeze(1)
    return X, pos, edge_index


def graph_build_split(data, node_mask: torch.Tensor):
...
def gnn_score(coalition: list, data: Data, value_func: Callable,
    ...
    mask[coalition] = 1.0
    ret_x, ret_pos, ret_edge_index = subgraph_build_func(data, mask)
    mask_data = Data(x=ret_x, pos=ret_pos, edge_index=ret_edge_index)
    mask_data = Batch.from_data_list([mask_data])
    score = value_func(data) - value_func(mask_data)
