...
class LRIGaussian(BaseRandom):

    def __init__(self, clf, extractor, criterion, config):
        super().__init__()
        self.name = 'lri_gaussian'
        self.clf = clf
    ...
    def __loss__(self, pred_sigma, clf_logits, clf_labels, epoch):
    ...
    def forward(self, data, edge_attn=None):
        original_clf_logits = self.clf(data, edge_attn=edge_attn)
        return original_clf_logits

    def forward_pass(self, data, epoch, do_sampling):
        emb, edge_index = self.clf.get_emb(data)
        U = self.extractor(emb, batch=data.batch, pool_out_lig=None)
        ...
        if self.attn_constraint == "smooth_min":
           pred_sigma = self.smooth_min(edge_index, pred_sigma, U, sig1, sig2) 
        ...
        loss, loss_dict = self.__loss__(pred_sigma, masked_clf_logits, data.y, epoch)

        return loss, loss_dict, masked_clf_logits, 1-(pred_sigma.det()).reshape(-1)
