...
def main(args):
    ...
    for model_path in findSubFile(Path('log') / config_name / 'erm' / 'models', restrict='pt'):
        # args.method = method
        path_list = re.split(r'[\./-]', model_path) # /seed0-9epo-999auc.pt
        auc_name, seed_name = path_list[-2], path_list[-4] # the former five letters are seed0_
        name = auc_name + f'_{seed_name[-1]}'
        print(f'The model to load is {name}. ({path_list[-3]})')
        report_dict += [test_one_seed(args, load_dir=model_path)]
        indexes += [name]
    # print(json.dumps(report_dict, indent=4))

    df_path = Path('result') / config_name / 'model_variation'
